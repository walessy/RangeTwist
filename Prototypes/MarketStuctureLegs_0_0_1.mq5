...
int OnInit()
{
   ...
   SetIndexBuffer(6, OB, INDICATOR_CALCULATIONS);
   ...
   PlotIndexSetInteger(2, PLOT_ARROW, 159); // Entry long arrow code
   PlotIndexSetInteger(3, PLOT_ARROW, 159); // Entry short arrow code
...
//+------------------------------------------------------------------+
void CheckForEntryOpportunity(int i, const double &high[], const double &low[])
{
   ...
   if(UpArrow[i] != EMPTY_VALUE)
   {
      ...
      {
         // Entry opportunity for long
         EntryLongArrow[i] = low[i] - 10 * _Point;
      }
   }
   ...
   if(DownArrow[i] != EMPTY_VALUE)
   {
      ...
      {
         // Entry opportunity for short
         EntryShortArrow[i] = high[i] + 10 * _Point;
      }
   }
}
   ...
   for (i = BarsForRecalculation; i >= 0; i--)
     {
      ...
      CheckForEntryOpportunity(i, high, low);
     } // end of for (main)
